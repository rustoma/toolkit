...
const randomStringSource = "qwertyuiopasdfghjklzxcvbnmQWERTYUIOPASDFGHJKLZXCVBNM_+"
...
// randomString return a string of random characters of length n,
...
// Uplaoded file is a struct used to save information about an uploaded file
	...
	for _, fHeaders := range r.MultipartForm.File {
		for _, hdr := range fHeaders {
			uploadedFiles, err = func(uploadedFiles []*UploadedFile) ([]*UploadedFile, error) {
				...
				if !allowed {
					return nil, errors.New("the uplaoded file type is not permitted")
				}
...
// Slugify creating a slug from string
...
// in the brwoser window by setting Content-Disposition header. It also aloows slecification
// of the display name.
func (t *Tools) DownloadStaticFile(w http.ResponseWriter, r *http.Request, p, file, displayName string) {
	fp := path.Join(p, file)
	w.Header().Add("Content-Disposition", fmt.Sprintf("attachment; filename=\"%s\"", displayName))
...
// ReadJSON tries to read the body of the request and converts it from json into a go data variable
func (t *Tools) ReadJSON(w http.ResponseWriter, r *http.Request, data interface{}) error {
	...
	if err != nil {
		...
		case errors.As(err, &unmarshalTypeError):
			...
			return fmt.Errorf("body contains incorrect JSON type (at charater %d)", unmarshalTypeError.Offset)
		...
		case err.Error() == "http: request body too large":
			return fmt.Errorf("body must not be larger than %d bystes", maxBytes)
